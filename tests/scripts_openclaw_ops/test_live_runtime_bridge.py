...
BRIDGE = ROOT / "scripts/openclaw-ops/live_runtime_bridge.py"
...
class LiveRuntimeBridgeTests(unittest.TestCase):
    def _load_bridge_module(self):
        spec = util.spec_from_file_location("live_runtime_bridge", BRIDGE)
    ...
    def test_bridge_env_exports_connector_profile_marker_without_credentials(self):
        bridge = self._load_bridge_module()
        args = SimpleNamespace(home=Path("/tmp/home"), hermes_bin=Path("/tmp/bin/hermes"))
        with mock.patch.dict(os.environ, {"PROJECT_PIPELINE_CONNECTOR_PROFILE": "team"}, clear=True):
            env = bridge.bridge_env(args, Path("/tmp/profile"), "external_research")

        self.assertEqual("team", env["PROJECT_PIPELINE_CONNECTOR_PROFILE"])
        self.assertNotIn("CONNECTOR_SECRET", env)

    def test_stage_prompt_includes_generic_connector_guidance(self):
        bridge = self._load_bridge_module()
        args = SimpleNamespace(project_dir=Path("/repo"), profile="projectagent")
        with mock.patch.dict(
            os.environ,
            {"PROJECT_PIPELINE_CONNECTOR_PROFILE": "team", "PIPELINE_RUN_DIR": "/tmp/run"},
            clear=True,
        ):
            prompt = bridge.stage_prompt("external_research", args, "根据外部目录的模块总览列清单")

        self.assertIn("External source access contract", prompt)
        self.assertIn("Configured connector profile: team", prompt)
        self.assertIn("stale identifier", prompt)
        self.assertIn("Do not read or print connector credential stores", prompt)
    ...
    def test_staged_diff_secret_scan_allows_scanner_code_diff(self):
        ...
        diff = "\n".join(
            [
                "diff --git a/scripts/openclaw-ops/live_runtime_bridge.py b/scripts/openclaw-ops/live_runtime_bridge.py",
                "+++ b/scripts/openclaw-ops/live_runtime_bridge.py",
                "@@ -0,0 +1,4 @@",
    ...
    def test_external_research_prompt_forbids_file_edits_and_allows_local_only_pass(self):
        bridge = self._load_bridge_module()
        args = SimpleNamespace(project_dir=ROOT, profile="projectagent")
    ...
    def test_external_research_recovers_local_only_output_from_session_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            profile_dir = tmp / "profiles" / "projectagent"
    ...
    def test_external_research_synthesizes_local_only_evidence_when_hermes_empty_fails(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            run_dir = tmp / "run"
            run_dir.mkdir()
            (run_dir / "run_meta.json").write_text(json.dumps({"source_urls": ["discord:projectagent"]}), encoding="utf-8")
    ...
    def test_run_verification_reports_timeout_from_cli_argument(self):
        ...
        with mock.patch.dict(os.environ, {"PROJECT_PIPELINE_LIVE_BRIDGE_TEST_COMMAND": slow_command}, clear=False), redirect_stdout(out):
    ...
    def test_deployment_runs_configured_command_and_smoke(self):
        bridge = self._load_bridge_module()
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            args = SimpleNamespace(
                allow_deployment=True,
                deployment_command="python deploy.py",
                deployment_command_timeout_seconds=30,
                smoke_command=["curl -fsS http://127.0.0.1:9000/health"],
                project_dir=tmp,
                deploy_wait_seconds=0,
            )
            calls: list[str] = []

            def fake_run_command(command, cwd=None, shell=False, timeout=None):
            ...
            self.assertEqual(0, rc)
            self.assertEqual(["python deploy.py", "curl -fsS http://127.0.0.1:9000/health"], calls)
