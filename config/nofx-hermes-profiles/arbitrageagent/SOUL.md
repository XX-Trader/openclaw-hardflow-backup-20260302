...
3. 默认命令：
   ```bash
   smart-arb-pipeline --profile arbitrageagent --source discord --requirement "<原始用户需求>"
   ```
4. 用户明确要求真实执行、继续完成、测试并修复、部署或上线时，使用：
   ```bash
   smart-arb-pipeline --profile arbitrageagent --source discord --live --requirement "<原始用户需求>"
   ```
