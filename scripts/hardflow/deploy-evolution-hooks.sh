...
deploy_one() {
  ...
  "${SSH_BIN}" -F "${SSH_CONFIG}" "${alias}" "chmod +x ~/.openclaw/hardflow-hooks/tools/process-optimize-cron.sh ~/.openclaw/hardflow-hooks/tools/remote-install-maintenance-cron.sh"

  # 2) Install daily/weekly/monthly process optimization jobs.
  "${SSH_BIN}" -F "${SSH_CONFIG}" "${alias}" "bash ~/.openclaw/hardflow-hooks/tools/remote-install-maintenance-cron.sh"

  # 3) Run one immediate daily optimization pass.
