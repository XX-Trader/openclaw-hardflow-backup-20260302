...
class HumanOutputFormatTests(unittest.TestCase):
    ...
    def test_task_executor_human_view_uses_readable_task_summaries(self) -> None:
        ...
        self.assertIn("2026-03-14 08:53:02 UTC+8 任务执行器（10分钟）：首次发现 3 个未闭环任务。", output)
        self.assertIn("事项1：补齐项目索引治理方案，并确认是否纳入本周计划。", output)
        self.assertIn("事项2：梳理增量巡检输出格式，确认哪些字段给人看。", output)
        self.assertIn("事项3：整理任务执行器的人类摘要模板，避免只显示任务 ID。", output)
        self.assertIn("执行结论1：等待人工确认", output)
        self.assertIn("失败原因1：等待人工确认", output)
        self.assertIn("需要协助1：人工确认后才能继续执行", output)
