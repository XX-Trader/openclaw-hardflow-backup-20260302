#!/usr/bin/env python3
"""Domain-neutral entrypoint for project delivery pipeline runs."""
...
PROJECT_KEY = os.environ.get("PROJECT_PIPELINE_PROJECT_KEY", "generic-project").strip() or "generic-project"
RUNTIME_HOME = Path(
...
BRIDGE = OPS_DIR / "live_runtime_bridge.py"
...
DIRECT_RUN_RECOMMENDATION_PATTERNS = [
    ...
    for pattern in (
        ...
        r"(?:只读|查询|看看|状态|监控|health|status)",
    )
]
...
HIGH_RISK_PATTERNS = [
    ...
    for pattern in (
        ...
        r"(?:需要|要求|读取|查看|输出|打印|提交|上传|使用|修改|删除).{0,20}(?:密钥|凭证|tokens?(?!ized)(?![A-Za-z0-9_])|cookie|私钥|会话)",
    )
]
...
SAFE_DOCUMENTATION_HISTORY_PATTERNS = [
    ...
    for pattern in (
        ...
        r"(?:原因|处理|自动修复判断|判断)\s*[:=：].{0,300}(?:\\b|\(\?:|tokens?\(\?!ized\)|密钥|凭证|token|cookie).{0,300}",
...
PRE_REDACTED_MARKER_RE = re.compile(
    r"__PROJECT_PIPELINE_PRE_REDACTED__(?P<key>[A-Za-z0-9_-]+)(?P<sep>[:=])\[REDACTED\]__"
)
...
def redact_text(value: object) -> str:
    ...
    def hold_marker(match: re.Match) -> str:
        ...
        return f"__PROJECT_PIPELINE_HELD_MARKER_{len(held_markers) - 1}__"
    ...
    for index, marker in enumerate(held_markers):
        text = text.replace(f"__PROJECT_PIPELINE_HELD_MARKER_{index}__", marker)
    return text
...
def mark_pre_redacted_sensitive_markers(text: str) -> str:
    def replacement(match: re.Match, separator: str) -> str:
        ...
        return f"__PROJECT_PIPELINE_PRE_REDACTED__{key}{separator}[REDACTED]__"
...
def render_route_selection_card(*, source: str, profile: str, requirement: str) -> str:
    ...
    lines.extend(
        [
            ...
            "防误触发: 当前调用没有携带人工选择凭证，已阻止直接启动 `project-delivery-pipeline`。",
            "回答状态: 等待人工选择",
        ]
    )
    return "\n".join(lines)
...
def discord_route_choice_required(source: str) -> bool:
    if str(source or "").strip().lower() != "discord":
        return False
    return env_flag("PROJECT_PIPELINE_REQUIRE_DISCORD_ROUTE_CHOICE", True)
...
def repair_context_markdown(state: dict, attempt: int, risk: str, reasons: list[str]) -> str:
    ...
    return "\n".join(
        [
            "# Project Delivery Auto Repair Context",
            "",
            f"- Attempt: {attempt}",
...
def specified_agent_executor_command(cmd: list[str]) -> list[str]:
    ...
    if is_root and target_user and shutil.which("runuser"):
        ...
        if runtime_cmd and Path(runtime_cmd[0]).name.lower().startswith("python"):
            runtime_cmd[0] = str(os.environ.get("PROJECT_PIPELINE_SPECIFIED_AGENT_PYTHON", "") or "python3")
        return ["runuser", "-u", target_user, "--", *runtime_cmd]
    return cmd


def specified_agent_run_as_user() -> str:
    return str(os.environ.get("PROJECT_PIPELINE_SPECIFIED_AGENT_RUN_AS", "") or "").strip()
...
def bridge_command(stage: str, args: argparse.Namespace, reviewer_role: str | None = None) -> str:
    ...
    if stage == "deployment":
        command.append("--allow-deployment")
...
def default_live_bridge_args(args: argparse.Namespace, passthrough: list[str]) -> list[str]:
    ...
    injected: list[str] = []
    inject_deployment = not args.skip_deployment_command and requirement_requests_deployment(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the generic project delivery pipeline")
    ...
    parser.add_argument("--runtime-host", default=os.environ.get("PROJECT_PIPELINE_RUNTIME_HOST", "local"))
    parser.add_argument(
        "--route-choice",
        choices=sorted(VALID_ROUTE_CHOICES),
        default=os.environ.get("PROJECT_PIPELINE_ROUTE_CHOICE", ""),
        help="manual route selected by the Discord user; Discord pipeline execution requires a pipeline route",
    )
    parser.add_argument("--assignee", default=os.environ.get("PROJECT_PIPELINE_SPECIFIED_AGENT", ""), help="agent id used by the specified_agent route")
    parser.add_argument("--openclaw-bin", default=os.environ.get("PROJECT_PIPELINE_OPENCLAW_BIN", "openclaw"), help="OpenClaw binary used by task_executor_runner for specified_agent dispatch")
    parser.add_argument(
        "--specified-agent-timeout-seconds",
        type=int,
        default=int(os.environ.get("PROJECT_PIPELINE_SPECIFIED_AGENT_TIMEOUT_SECONDS", "1200")),
    ...
    parser.add_argument("--live-bridge-agent-mode", choices=["hermes", "echo"], default=os.environ.get("PROJECT_PIPELINE_LIVE_BRIDGE_AGENT_MODE", "hermes"))
    parser.add_argument("--live-bridge-provider", default=os.environ.get("PROJECT_PIPELINE_LIVE_BRIDGE_PROVIDER", "openai-codex"))
    parser.add_argument("--live-bridge-model", default=os.environ.get("PROJECT_PIPELINE_LIVE_BRIDGE_MODEL", "gpt-5.5"))
    parser.add_argument("--reviewer-a-provider", default=os.environ.get("PROJECT_PIPELINE_REVIEWER_A_PROVIDER"))
    parser.add_argument("--reviewer-a-model", default=os.environ.get("PROJECT_PIPELINE_REVIEWER_A_MODEL"))
    parser.add_argument("--reviewer-b-provider", default=os.environ.get("PROJECT_PIPELINE_REVIEWER_B_PROVIDER") or DEFAULT_REVIEWER_B_PROVIDER)
    parser.add_argument("--reviewer-b-model", default=os.environ.get("PROJECT_PIPELINE_REVIEWER_B_MODEL") or DEFAULT_REVIEWER_B_MODEL)
    parser.add_argument("--reviewer-fallback-models", default=os.environ.get("PROJECT_PIPELINE_REVIEWER_FALLBACK_MODELS") or DEFAULT_REVIEWER_FALLBACK_MODELS)
    parser.add_argument("--live-bridge-timeout-seconds", type=int, default=int(os.environ.get("PROJECT_PIPELINE_LIVE_BRIDGE_TIMEOUT_SECONDS", "1800")))
    parser.add_argument(
        "--live-bridge-verification-command-timeout-seconds",
        type=int,
        default=int(os.environ.get("PROJECT_PIPELINE_LIVE_BRIDGE_VERIFICATION_COMMAND_TIMEOUT_SECONDS", "300")),
        help="per-command timeout used inside the live verification bridge",
    )
    parser.add_argument("--live-bridge-agent-max-turns", type=int, default=int(os.environ.get("PROJECT_PIPELINE_LIVE_BRIDGE_AGENT_MAX_TURNS", "24")))
    parser.add_argument("--live-bridge-code-max-turns", type=int, default=int(os.environ.get("PROJECT_PIPELINE_LIVE_BRIDGE_CODE_MAX_TURNS", "60")))
    parser.add_argument("--code-agent", choices=["backend-dev", "frontend-dev"], default=normalize_code_agent(os.environ.get("PROJECT_PIPELINE_CODE_AGENT", "")), help="workflow owner for code execution; defaults to inferred backend/frontend owner")
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=env_flag("PROJECT_PIPELINE_SKIP_GIT_PUBLISH_COMMAND", False),
    ...
    parser.add_argument("--chat-stage-limit", type=int, default=int(os.environ.get("PROJECT_PIPELINE_CHAT_STAGE_LIMIT", "20")))
    parser.add_argument("--chat-command-limit", type=int, default=int(os.environ.get("PROJECT_PIPELINE_CHAT_COMMAND_LIMIT", "24")))
    parser.add_argument(
        "--chat-include-command-output",
        action="store_true",
        default=env_flag("PROJECT_PIPELINE_CHAT_INCLUDE_COMMAND_OUTPUT", False),
        help="include compact command stdout/stderr excerpts in chat cards; disabled by default",
    )
    parser.add_argument(
        "--chat-show-key-artifacts",
        action="store_true",
        default=env_flag("PROJECT_PIPELINE_CHAT_SHOW_KEY_ARTIFACTS", False),
        help="append the legacy key artifact name list to the final chat card",
    )
    parser.add_argument(
        "--progress-interval-seconds",
        type=int,
        default=int(os.environ.get("PROJECT_PIPELINE_PROGRESS_INTERVAL_SECONDS", "60")),
        help="print an in-flight progress card every N seconds; set to 0 to disable",
    )
    parser.add_argument(
        "--progress-stage-limit",
        type=int,
        default=int(os.environ.get("PROJECT_PIPELINE_PROGRESS_STAGE_LIMIT", "8")),
        help="number of recent stages to include in in-flight progress cards",
    )
    parser.add_argument(
        "--progress-command-limit",
        type=int,
        default=int(os.environ.get("PROJECT_PIPELINE_PROGRESS_COMMAND_LIMIT", "3")),
        help="number of recent command outputs to include in in-flight progress cards",
    )
    parser.add_argument(
        "--auto-repair-attempts",
        type=int,
        default=int(os.environ.get("PROJECT_PIPELINE_AUTO_REPAIR_ATTEMPTS", "4")),
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=env_flag("PROJECT_PIPELINE_HUMAN_RISK_CONFIRMED", False),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run or option_present(passthrough, "--dry-run"):
        parser.error("dry-run is disabled for project-delivery-pipeline; execution requests always run live")
    args.live = True

    profile = args.profile or "deliveryagent"
    ...
    while state:
        ...
        if not do_repair:
            ...
            break
        repair_attempts += 1
        repair_env.pop("PROJECT_PIPELINE_ENTRY_REPAIR_CONTEXT_FILE", None)
        ...
        repair_env["PROJECT_PIPELINE_ENTRY_REPAIR_CONTEXT"] = context_text
        repair_env["PIPELINE_REPAIR_ATTEMPT"] = str(repair_attempts)
        if context_file is not None:
            repair_env["PROJECT_PIPELINE_ENTRY_REPAIR_CONTEXT_FILE"] = str(context_file)
