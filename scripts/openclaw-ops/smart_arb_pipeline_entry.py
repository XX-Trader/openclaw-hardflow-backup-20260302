...
HIGH_RISK_PATTERNS = [
    ...
    for pattern in (
        ...
        r"\bforce\s+push\b",
        r"(?:需要|要求|读取|查看|输出|打印|提交|上传|使用|修改|删除).{0,20}(?:密钥|凭证|(?<!stock_)tokens?(?!ized)(?![A-Za-z0-9_])|cookie|私钥|会话)",
...
SAFE_DOCUMENTATION_HISTORY_PATTERNS = [
    ...
    for pattern in (
        ...
        r"(?:原因|处理|自动修复判断|判断)\s*[:=：].{0,300}(?:\\b|\(\?:|\?<!stock_|withdraw|place\\s\+orders|submit\\s\+orders|tokens?\(\?!ized\)|密钥|凭证|token|cookie).{0,300}",
