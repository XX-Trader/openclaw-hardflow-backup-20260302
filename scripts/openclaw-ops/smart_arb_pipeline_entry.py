...
SAFE_DOCUMENTATION_HISTORY_PATTERNS = [
    ...
    for pattern in (
        ...
        r"(?:触发点|原因|自动修复判断).{0,80}(?:风险规则|high|高风险|文本中仍出现).{0,160}(?:真实交易|实盘交易|下单|划转|转账|提现|出金|资金操作|凭证|密钥|token|cookie)",
        r"(?:没有|未|不曾).{0,20}(?:启动|执行|进行|发生|完成)?(?:真实交易|实盘交易|下单|划转|转账|提现|出金|资金操作)",
    )
]
