...
KNOWN_SCHEDULE_METADATA: dict[str, dict[str, Any]] = {
    ...
    "project_index_maintainer_30m": {
        ...
        "purpose": "维护项目索引与上下文入口",
        "flow_summary": "project_index_maintainer.py -> 刷新项目索引 -> 回写 task-center",
        "required_skills_or_runtime": ["project-registry.json", "task_center.db"],
        "outputs": ["项目索引", "上下文任务"],
        "human_visibility": "默认静默",
        "delivery_mode": "none",
        "health_signals": ["索引文件可写", "项目发现正常"],
    ...
    "ops_governance_evolution_incremental": {
        ...
        "purpose": "扫描治理问题并生成优化任务",
        "flow_summary": "governance_evolution_runner.py -> 扫描 hooks/plugins/workflows -> 派生任务",
    ...
    "reviewer_git_update_hourly": {
        ...
        "capability": "hourly_git_review",
        "purpose": "小时级 git 审查与 PR 检查",
        "flow_summary": "reviewer_cron_runner.py --mode hourly_git",
