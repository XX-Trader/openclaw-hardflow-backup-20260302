#!/usr/bin/env python3
"""Install or update one git-sync job in OpenClaw jobs.json."""
...
DEFAULT_JOB_ID = "5dd96c0a-5cd2-4b31-b9a6-75f6ef4f3339"
DEFAULT_JOB_NAME = "ops_git_sync_push"
DEFAULT_DESCRIPTION = "Auto sync local repo and push self-evolution changes to remote git"
...
def build_message(command: str) -> str:
    return build_scheduled_runner_message(
        ...
        role="git sync scheduled runner",
...
def build_runner_command(
    *,
    ...
    cmd = (
        ...
        f"--commit-prefix \"{q(str(commit_prefix or '').strip())}\""
    )
    if str(branch).strip():
        cmd += f" --branch \"{q(str(branch).strip())}\""
    cmd += " --auto-pull" if auto_pull else " --no-auto-pull"
    cmd += " --push" if push else " --no-push"
    for prefix in include_prefixes:
        text = str(prefix or "").strip()
        if text:
            cmd += f" --include-prefix \"{q(text)}\""
    for prefix in exclude_prefixes:
        text = str(prefix or "").strip()
        if text:
            cmd += f" --exclude-prefix \"{q(text)}\""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Install one git-sync job")
    ...
    parser.add_argument("--runner-py", default=str(home / ".openclaw/ops/git_sync_push_runner.py"))
        ...
        "payload": {
            ...
            "message": build_message(
                build_runner_command(
                    ...
                    commit_prefix=str(args.commit_prefix).strip() or "chore(self-evolution): sync updates",
                    auto_pull=bool(args.auto_pull),
                    push=bool(args.push),
                    include_prefixes=list(args.include_prefix or []),
