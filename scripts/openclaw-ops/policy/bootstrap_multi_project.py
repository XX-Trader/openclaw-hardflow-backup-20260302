...
def bootstrap_project(
    ...
    workflow_io_dir = task_dir / "workflow-io"
    agent_log_root = task_dir / "agents"
    ensure_dir(task_dir)
    ensure_dir(workflow_io_dir)
    ensure_dir(agent_log_root)

    policy_env = workflow_dir / "policy.env"
    task_db = task_dir / "task_center.db"
    registry_file = workflow_dir / "project-index" / "project-registry.json"
    ...
    policy_env_content = "\n".join(
        [
            ...
            f"OPENCLAW_HOME={shlex.quote(str(openclaw_home))}",
            f"OPENCLAW_CONFIG={shlex.quote(str(openclaw_home / 'openclaw.json'))}",
            f"TASK_CENTER_DIR={shlex.quote(str(task_dir))}",
            f"WORKFLOW_IO_DIR={shlex.quote(str(workflow_io_dir))}",
            f"AGENT_LOG_ROOT={shlex.quote(str(agent_log_root))}",
            f"PROJECT_REGISTRY={shlex.quote(str(registry_file))}",
            "POLICY_ENFORCER_ENABLED=1",
            "POLICY_ENFORCER_STRICT=1",
            f"OPENCLAW_POLICY_ROOT={shlex.quote(str(shared_policy_dir))}",
            ...
            f"TOKEN_PRICING_FILE={shlex.quote(str(shared_policy_dir / 'token-pricing.json'))}",
            f"PROJECT_INDEX_MAINTAINER_PY={shlex.quote(str(shared_policy_dir / 'project_index_maintainer.py'))}",
            f"PROJECT_INDEX_REGISTRY={shlex.quote(str(registry_file))}",
            "",
        ]
    )
    atomic_write_text(
