#!/usr/bin/env python3
"""Domain-neutral live evidence bridge for the project delivery pipeline."""
...
PROJECT_DIR = Path(os.environ.get("PROJECT_PIPELINE_PROJECT_DIR", str(REPO_ROOT))).expanduser()
RUNTIME_HOME = Path(
    os.environ.get("HARDFLOW_RUNTIME_HOME", str(Path.home() / ".hardflow-runtime"))
).expanduser()
HERMES_BIN = Path(os.environ.get("PROJECT_PIPELINE_HERMES_BIN", "hermes"))
...
EXTERNAL_LOOKUP_REQUIRED_RE = re.compile(
    ...
    r"version-sensitive|版本敏感|依赖升级|upgrade dependency|platform rule|平台规则"
)
    ...
    return "\n".join(
        [
            ...
            *(f"- `{item}`" for item in source_urls or ["not_supplied"]),
            ...
            "",
            "安全边界：未读取或打印凭证；本阶段不修改文件，只返回 research evidence，由 runner 写入 `research_report.md`。",
...
def repair_context_text() -> str:
    for name in ("PIPELINE_REPAIR_CONTEXT_FILE", "PROJECT_PIPELINE_ENTRY_REPAIR_CONTEXT_FILE"):
        path = env_path(name, Path(""))
        if path and str(path) != "." and path.exists():
            return read_text(path)
    for name in ("PIPELINE_REPAIR_CONTEXT", "PROJECT_PIPELINE_ENTRY_REPAIR_CONTEXT"):
        value = os.environ.get(name, "").strip()
        if value:
            return value
    return ""
...
def connector_profile() -> str:
    """Return an optional connector profile name without reading credentials."""

    return os.environ.get("PROJECT_PIPELINE_CONNECTOR_PROFILE", "").strip()
...
def bridge_env(args: argparse.Namespace, profile_dir: Path, stage: str = "") -> dict[str, str]:
    ...
    if profile:
        env["PROJECT_PIPELINE_CONNECTOR_PROFILE"] = profile
    path_parts = [str(args.hermes_bin.parent), "/usr/local/bin", "/usr/bin", "/bin"]
    existing_path = env.get("PATH", "")
    env["PATH"] = ":".join([part for part in path_parts if part] + ([existing_path] if existing_path else []))
    return env


def external_source_guidance() -> str:
    profile = connector_profile()
    profile_note = f"Configured connector profile: {profile}." if profile else "No connector profile was selected."
    return f"""
External source access contract for all pipeline agents:
- Use a repository-supported connector or CLI when the requirement references an external data source.
- Discover current resources, schemas, collections, and views before treating a stale identifier as a terminal failure.
- A login redirect alone is not evidence that a source is unavailable; prefer the configured connector and report its concrete result.
- {profile_note}
- Do not read or print connector credential stores, tokens, cookies, app secrets, OAuth state, API keys, or private keys.
""".strip()
...
def stage_prompt(stage: str, args: argparse.Namespace, requirement: str) -> str:
    ...
    common = f"""
You are running one non-interactive stage of a project delivery pipeline.
...
Use available browser/search tools when the requirement depends on external facts, third-party APIs, platform behavior, library versions, deployment patterns, or unknown best practices.
...
4. reviewer gives final acceptance criteria, likely files, verification commands, and concrete failure reasons that must be fixed before implementation continues.
...
"""
        ...
        focus = (
            ...
            else "backend, script, service, API, and application-runtime changes"
        )
        specific = """
...
""".format(role=role, focus=focus)
        ...
        if stage == "requirements_review":
            ...
            focus = "requirements clarity, scope, acceptance criteria, and concrete developer handoff"
...
For solution review, validate the structured `delivery_plan.json` contract first; `solution.md` is only the human-readable rendering. Use `graphify_context.md` and `graphify_scope_validation.md` to challenge missing related modules/tests and respect its policy: warning by default; block only with concrete evidence that the plan cannot be implemented or verified.
Every review stage is a hard feedback loop. If the work is not ready, return `requires_revision` with exact failure reasons that the developer can fix, then review again until it passes.
Do not require artificial task-splitting granularity; review the whole accepted requirement and block only with concrete evidence.
...
def effective_reviewer_provider(args: argparse.Namespace, role: str) -> str:
    ...
    if suffix:
        value = os.environ.get(f"PROJECT_PIPELINE_REVIEWER_{suffix}_PROVIDER", "").strip()
        if value:
            return value
...
def effective_reviewer_model(args: argparse.Namespace, role: str) -> str:
    ...
    suffix = "A" if role == "reviewer-a" else "B" if role == "reviewer-b" else ""
    if suffix:
        value = os.environ.get(f"PROJECT_PIPELINE_REVIEWER_{suffix}_MODEL", "").strip()
        if value:
            return value
...
def run_echo_stage(stage: str, reviewer_role: str = "") -> int:
    if stage == "requirements_review":
        print("# Project Delivery Live Bridge Echo Requirements Review")
        ...
        print("# Project Delivery Live Bridge Echo Solution Review")
        print("Final verdict: ready_for_implement")
        print(f"Reviewer role: {reviewer_role or 'reviewer-a'}")
        print("Reviewer provider: echo")
        print(f"Reviewer model: echo-{reviewer_role or 'reviewer-a'}")
    elif stage == "code_review":
        print("# Project Delivery Live Bridge Echo Review")
        print("Final verdict: pass")
        print(f"Reviewer role: {reviewer_role or 'reviewer-a'}")
        print("Reviewer provider: echo")
        print(f"Reviewer model: echo-{reviewer_role or 'reviewer-a'}")
    elif stage == "deployment":
        print("# Project Delivery Live Bridge Echo Deployment")
        print("- Internal API restart skipped in echo mode.")
    else:
        print(f"# Project Delivery Live Bridge Echo {stage}")
...
def verification_commands(args: argparse.Namespace) -> list[str]:
    ...
    test_command = os.environ.get("PROJECT_PIPELINE_LIVE_BRIDGE_TEST_COMMAND", "").strip()
    if test_command:
        ...
        targets = [
            ...
            for name in ("src", "app", "lib", "services", "scripts", "tests")
            if (args.project_dir / name).exists()
        ]
        if targets:
...
def run_verification(args: argparse.Namespace) -> int:
    sections: list[str] = ["# Project Delivery Live Verification"]
    ok = True
...
def run_deployment(args: argparse.Namespace) -> int:
    sections = ["# Project Delivery Live Deployment"]
    if not args.allow_deployment:
        sections.append("- Deployment execution flag is absent.")
        sections.append("LIVE_BRIDGE_STAGE: deployment")
        sections.append("LIVE_BRIDGE_STATUS: fail")
        print("\n\n".join(sections))
        return 2

    deployment_command = str(args.deployment_command or "").strip()
    if not deployment_command:
        sections.append("- No project-specific deployment command was configured.")
        sections.append("LIVE_BRIDGE_STAGE: deployment")
        sections.append("LIVE_BRIDGE_STATUS: fail")
        print("\n\n".join(sections))
        return 3
    ...
    deploy_proc = run_command(
        ...
        shell=True,
        timeout=int(args.deployment_command_timeout_seconds),
    )
    sections.append(command_block("Deployment command", deployment_command, deploy_proc))
    ...
    smoke_commands = [str(item).strip() for item in args.smoke_command if str(item).strip()]
    if not smoke_commands:
        smoke_commands = [
            item.strip()
            for item in os.environ.get("PROJECT_PIPELINE_SMOKE_COMMANDS", "").split(";;")
            if item.strip()
        ]

    if not smoke_commands:
        sections.append("- No smoke command configured; deployment command result is the available evidence.")
        sections.append("LIVE_BRIDGE_STAGE: deployment")
...
def run_memory_writeback(args: argparse.Namespace) -> int:
    ...
    project_key = os.environ.get("PIPELINE_PROJECT_KEY", "").strip() or "generic-project"
...
def run_git_publish(args: argparse.Namespace) -> int:
    sections = ["# Project Delivery Git Publish"]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Project delivery live pipeline evidence bridge")
    ...
    parser.add_argument("--agent-mode", choices=["hermes", "echo"], default=os.environ.get("PROJECT_PIPELINE_LIVE_BRIDGE_AGENT_MODE", "hermes"))
    parser.add_argument("--profile", default=os.environ.get("PROJECT_PIPELINE_LIVE_BRIDGE_PROFILE", "deliveryagent"))
    parser.add_argument(
        "--project-dir",
        type=Path,
        default=env_path("PIPELINE_AGENT_REPO_DIR", env_path("PROJECT_PIPELINE_PROJECT_DIR", PROJECT_DIR)),
    )
    parser.add_argument("--runtime-home", type=Path, default=env_path("PROJECT_PIPELINE_HERMES_RUNTIME_HOME", RUNTIME_HOME))
    parser.add_argument("--home", type=Path, default=env_path("PROJECT_PIPELINE_HOME", Path.home()))
    parser.add_argument("--hermes-bin", type=Path, default=env_path("PROJECT_PIPELINE_HERMES_BIN", HERMES_BIN))
    parser.add_argument("--provider", default=os.environ.get("PROJECT_PIPELINE_LIVE_BRIDGE_PROVIDER", "openai-codex"))
    parser.add_argument("--model", default=os.environ.get("PROJECT_PIPELINE_LIVE_BRIDGE_MODEL", "gpt-5.5"))
    ...
    parser.add_argument(
        ...
        default=os.environ.get("PROJECT_PIPELINE_REVIEWER_FALLBACK_MODELS", DEFAULT_REVIEWER_FALLBACK_MODELS),
        help="comma-separated provider/model reviewer fallback chain used when a reviewer provider is unavailable",
    )
    parser.add_argument("--max-turns", type=int, default=int(os.environ.get("PROJECT_PIPELINE_LIVE_BRIDGE_MAX_TURNS", "24")))
    parser.add_argument("--allow-yolo", action="store_true")
    parser.add_argument("--skip-tests", action="store_true")
    parser.add_argument("--python-bin", type=Path, default=env_path("PROJECT_PIPELINE_PYTHON_BIN", Path(sys.executable)))
    parser.add_argument(
        "--verification-command-timeout-seconds",
        type=int,
        default=int_env("PROJECT_PIPELINE_LIVE_BRIDGE_VERIFICATION_COMMAND_TIMEOUT_SECONDS", 300),
    )
    parser.add_argument("--allow-deployment", action="store_true")
    parser.add_argument("--deployment-command", default=os.environ.get("PROJECT_PIPELINE_DEPLOYMENT_COMMAND", ""))
    parser.add_argument("--smoke-command", action="append", default=[])
    parser.add_argument(
        "--deployment-command-timeout-seconds",
        type=int,
        default=int_env("PROJECT_PIPELINE_DEPLOYMENT_COMMAND_TIMEOUT_SECONDS", 300),
    )
    parser.add_argument("--deploy-wait-seconds", type=int, default=int(os.environ.get("PROJECT_PIPELINE_DEPLOY_WAIT_SECONDS", "30")))
    parser.add_argument("--git-remote", default=os.environ.get("PROJECT_PIPELINE_GIT_REMOTE", "origin"))
    parser.add_argument("--git-branch", default=os.environ.get("PROJECT_PIPELINE_GIT_BRANCH", "main"))
    parser.add_argument("--git-push-timeout-seconds", type=int, default=int_env("PROJECT_PIPELINE_GIT_PUSH_TIMEOUT_SECONDS", 120))
    return parser
