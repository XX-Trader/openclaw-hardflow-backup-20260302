...
description_zh: "cron-mastery技能，详见 SKILL.md"
---
...
While Cron is precise, execution depends on the **Gateway Heartbeat** (typically every 10-60s). A job set for `:00` seconds will fire on the first "tick" after that time. Expect up to ~30s variance depending on your gateway config.
...
```json
{
  ...
  "delivery": { "mode": "announce", "channel": "telegram", "to": "<YOUR_TELEGRAM_USER_ID>" }
}
```
