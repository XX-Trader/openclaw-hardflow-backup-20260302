...
事实：`specified_agent` 不再是“非 pipeline route 跳过”占位。Discord 用户选择 `specified_agent` 并提供 `--assignee <agent-id>` 后，入口会创建 `specified_agent_dispatch` Task Center 任务，调用 `task_executor_runner.py` 分配给该 agent，并生成 `# nofx 指定 agent 执行状态` 卡。`coding_workflow` 的命令报告会抽取 live bridge / executor 输出中的 `session_id`、`runId`、`sessionKey`，写入 `command-runs/*.json`、`pipeline_state.agent_invocations`、Task Center payload 和最终 Discord 状态卡。状态卡明确展示谁被调用、session/run id、当前阶段、是否完成和失败原因。
证据：新增 `run_specified_agent_route()`、`specified_agent_dispatch` Task Center 创建逻辑、executor 调用、`agent_invocations` 聚合、`LIVE_BRIDGE_AGENT_SESSION_ID` 输出、Task executor `agent_run_id/session_key` 回写、状态卡 `被调用 agent 明细`。新增回归测试覆盖指定 agent 创建任务并渲染 id、pipeline state 收集 session/run id、状态卡展示 runtime refs、Hermes session id 抽取。
最后验证：2026-04-29 02:31，本地 `python -m pytest tests/scripts_openclaw_ops/test_smart_arb_pipeline_entry.py -q` 41 项与 43 个 subtests OK；新增定向 5 项 OK；`python -m py_compile` 覆盖 4 个改动脚本 OK；`git diff --check` OK。当前记录为本仓实现事实，nofx live 安装需以后续部署记录为准。
