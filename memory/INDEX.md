...
- `delivery_plan.json` 现在承担任务拆分职责：多事项需求会拆成 `scope_slices`，第一块为本轮 `current`，其余标记 `deferred`，避免 backlog runner 或人工入口一次执行过重任务；凭证泄露、目标不清、备份/审计失败和需求不清仍停 human inbox 或回问用户，资金/交易/破坏性动作则进入 `execution_guard.json`。
...
- nofx `solution_review` 现在是方案质量软门禁：任一有效 reviewer 给出 `requires_revision` 时，状态卡、`solution_review.md` 和 failure summary 必须写出 reviewer-a/reviewer-b 的具体未通过原因、两路讨论/挑战和合并后的 `Complete Revision Plan`；普通计划质量 blocker 会写入 `solution_review_soft_gate.md` 并注入 `code_execution`，后续由 `code_review` 硬性验证。凭证/secret/cookie/auth-state 泄露、无 reviewer 输出、明确绕过安全边界、破坏性目标不明确或备份/审计失败仍在实现前硬停；force push 和破坏性生产变更本身进入 `execution_guard.json`。
- 自动修复与 pre-execution 风险扫描都按子句剥离“不得泄露凭证 / 不启动真实交易 / 不下单不划转”等纯否定式安全边界；已脱敏字段如果表达 `Need api_key=[REDACTED]`、`Need Authorization: [REDACTED]` 或 `Need session_id=[REDACTED]` 仍按 hard block，`No need for ...` / `Do not need ...` 这类否定噪音可自动回流。混合句里正向要求读取/打印/提交凭证仍 hard block；正向启用实盘、真实下单、资金操作或破坏性命令进入 `guarded_execute`，由 `execution_guard.json` 约束后继续。
...
- 最新 nofx 安装记录：2026-05-07 15:32 已安装 `solution_review` 软门禁批次，远端 hardflow 仓库 `HEAD=c746cf3`、`HEAD...origin/main=0 0`，runtime installer 返回 `ok=true`、`changed=true`。远端 runner 65 项、live bridge 41 项、entry 49 项和 `compileall` OK；安装态已命中 `solution_review_can_soft_continue`、`solution_review_soft_gate.md` 与 `soft_continue`。15:02 的 reviewer blocker 合并批次、14:01 的 `external_research` 本地证据降级批次仍有效；2026-05-06 23:40 的高风险确认 echo smoke `cli-spreadagent-20260506T153935576001Z` 仍是策略高风险确认链路的最新 live smoke。详见 `DEPLOYMENT.md`、`RUNBOOK.md`、`TASK_HISTORY.md`。
