...
- 最新 nofx 安装记录：2026-05-07 12:53 已安装方案文件级计划与风险误判修复批次，远端 hardflow 仓库 `HEAD=8127001`、`HEAD...origin/main=0 0`。runtime installer 返回 `ok=true`、`changed=true`；远端 `compileall`、`test_project_delivery_pipeline_runner` 62 项和 `test_smart_arb_pipeline_entry` 49 项 OK；`smart-arb-pipeline --help` 已显示 `--route-choice`、`--reviewer-fallback-models` 与 `--human-risk-confirmed`；内控 API smoke 通过，两个 Discord gateway 均为 `running/connected`。真实 run `discord-spreadagent-20260507T040201861377Z` replay 产出 `bad_targets=[]`、14 条验收命令和无 Graphify block。上一批 2026-05-06 23:40 的高风险确认 echo smoke `cli-spreadagent-20260506T153935576001Z` 仍是策略高风险确认链路的最新 live smoke。详见 `DEPLOYMENT.md`、`RUNBOOK.md`、`TASK_HISTORY.md`。
