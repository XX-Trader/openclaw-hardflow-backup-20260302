...
- 2026-04-28 本仓 nofx Discord profile 模板已升级为普通沟通/独立协作与高权限工作流维护分流并同步到 live profile：普通业务代码修改、依赖安装、业务重启、业务部署和业务提交推送仍走 coordinator pipeline；但修复 hardflow workflow/runtime/profile 本身时，profile 不再递归启动同一条 `smart-arb-pipeline`，而是进入高权限工作流维护模式，直接切到 `/home/arbops/projects/openclaw-hardflow-backup-20260302` 修改工作流宿主、测试、安装 runtime，并在无法启动独立 reviewer 时标记 `review=pending_external`。live 备份后缀为 `manual-route-20260428T151420Z`，两个 Discord gateway 已重启且 connected。
...
- 本仓库已新增 `backlog_runner.py` 与 `backlog_runner_30m` cron，但 2026-04-28 起默认进入“手动链路选择”阶段：系统只推荐直接运行、需求探讨、指定 agent、编码工作流或 TODO 自动候选，用户确认后才执行。到期 TODO 即使低风险，也先进入 `human_inbox.py` 路线选择；只有选择为 `coding_workflow` / `todo_auto_candidate` 等 pipeline 动作后，backlog runner 才会调用 `smart-arb-pipeline`。`cron/jobs.json` 的 announce / failureAlert 默认投递到 spreadagent Discord 群 `1494595527181078578`。
...
- 最新 nofx 安装记录：2026-04-28 23:15 已把最新 `origin/main` 提交 `d2e530b7` 对齐并安装到 `/home/arbops/.hermes/ops`，远端 hardflow 仓库 HEAD 为 `d2e530b7`，`HEAD...origin/main=0 0` 且工作树 clean。runtime installer 返回 `ok=true`、`changed=true`，新增安装 `policy_route_selection.py`；远端 `git diff --check`、7 个脚本 `py_compile`、`compileall`、19 项定向单测、`/home/arbops/.local/bin/smart-arb-pipeline --help`、cron job 数 12 / `memtidy_hits=0`、gateway state、内控 API 和 echo smoke `install-smoke-arbitrageagent-20260428T151514657470Z` 均通过。本轮已同步两个 live profile SOUL 并重启 Discord gateway；详见 `DEPLOYMENT.md`、`RUNBOOK.md`、`TASK_HISTORY.md`。
