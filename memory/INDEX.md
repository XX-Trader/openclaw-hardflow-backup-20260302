...
- nofx `solution_review` 现在是方案质量软门禁：任一有效 reviewer 给出 `requires_revision` 时，状态卡、`solution_review.md` 和 failure summary 必须写出 reviewer-a/reviewer-b 的具体未通过原因、两路讨论/挑战和合并后的 `Complete Revision Plan`；普通计划质量 blocker 会写入 `solution_review_soft_gate.md` 并注入 `code_execution`，后续由 `code_review` 硬性验证。凭证/secret/cookie/auth-state、force push、破坏性生产数据、无 reviewer 输出或明确绕过安全边界仍在实现前硬停。
