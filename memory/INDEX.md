...
- 最新 nofx 安装记录：2026-05-07 10:58 已安装方案包范围与风险扫描修复批次，远端 hardflow 仓库 `HEAD=5d04f55`、`HEAD...origin/main=0 0`。runtime installer 返回 `ok=true`、`changed=true`；远端 `compileall` 与 12 项关键回归 unittest OK；`smart-arb-pipeline --help` 已显示 `--reviewer-fallback-models` 与 `--human-risk-confirmed`；内控 API smoke 通过，两个 Discord gateway 均为 `running/connected`。上一批 2026-05-06 23:40 的高风险确认 echo smoke `cli-spreadagent-20260506T153935576001Z` 仍是策略高风险确认链路的最新 live smoke。详见 `DEPLOYMENT.md`、`RUNBOOK.md`、`TASK_HISTORY.md`。
...
- 2026-05-07 起 `delivery_plan` 的目标文件解析会过滤 workflow 宿主 basename、低信任 `scripts/openclaw-ops/*`、组合路径和否定上下文；Graphify / pre-execution 风险扫描只看可执行意图，不把安全边界、状态摘要或验证命令里的关键词当成真实高风险动作。
