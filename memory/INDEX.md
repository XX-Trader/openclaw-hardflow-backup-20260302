...
- `delivery_plan.json.target_files` 现在只把用户原始需求/修复上下文中的显式路径作为高可信目标；review / research / project memory 仅作低信任补充并过滤 `.workflow`、runtime host、Task Center、agent workspace、command report 和项目记忆控制文件，简单任务没有可靠业务文件时保持 `discovery_required=true`。
