...
- 2026-04-29 本仓 nofx Discord profile 模板和 `smart_arb_pipeline_entry.py` 已升级为“所有任务先执行链路选择 + Discord profile 最高权限调度入口 + route-choice 入口硬门禁”：只读查询、普通沟通、方案讨论、安全仓库同步、业务代码修改、部署排障、TODO 推进和 hardflow workflow/runtime/profile 自修都必须先发“执行链路选择”卡；用户选择 `direct_run` 后由当前 Discord profile 作为最高权限 operator 直接处理，选择 `coding_workflow` / `todo_auto_candidate` 后才启动 coordinator pipeline。入口脚本现在要求 Discord source 携带有效 `--route-choice`：缺失时只返回选择卡；`specified_agent` 必须带 `--assignee <agent-id>`，并创建 Task Center 任务调用指定 agent；`coding_workflow` 会把真实 agent session/run id 写入 command-runs、Task Center 和状态卡。当前 nofx 已安装提交 `8d952c0d`，两个 Discord gateway 已重启且 connected；本仓后续新增的 specified_agent/session-id 批次是否已安装，以最新部署记录为准。
...
- 最新 nofx 安装记录：2026-04-29 01:24 已把 `origin/main` 提交 `8d952c0d` 对齐并安装到 `/home/arbops/.hermes/ops`，远端 hardflow 仓库 `HEAD...origin/main=0 0` 且工作树 clean。runtime installer 返回 `ok=true`、`changed=true`；远端 `git diff --check`、`py_compile`、`compileall`、41 项定向单测、`/home/arbops/.local/bin/smart-arb-pipeline --help`、gateway state、内控 API 和缺失 `--route-choice` 的选择卡 smoke 均通过。本轮已同步两个 live profile SOUL 并重启 Discord gateway；详见 `DEPLOYMENT.md`、`RUNBOOK.md`、`TASK_HISTORY.md`。
