...
- 最新 nofx 安装记录：2026-04-27 23:17 已把提交 `067fbc43` 拉到 `/home/arbops/projects/openclaw-hardflow-backup-20260302` 并运行 runtime installer；安装态 `/home/arbops/.hermes/ops` 与仓库源码 hash 对齐，包含 `pipeline_runner.py`、`smart_arb_pipeline_entry.py`、`smart_arb_live_bridge.py` 的密钥扫描收敛版本。远端 `py_compile`、`compileall`、98 项定向单测、`smart-arb-pipeline --help`、cron 检查、gateway state、内控 API smoke 和 echo Task Center smoke 均通过；安装前已备份 runtime 目标文件到 `/home/arbops/.hermes/ops/install/backups/pre-hardflow-install-20260427T151242Z`，详见 `RUNBOOK.md` / `DEPLOYMENT.md`。
