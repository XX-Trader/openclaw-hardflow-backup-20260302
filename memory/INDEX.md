...
- nofx Discord 状态卡会读取 `command-runs/*.json`，展示阶段命令状态、阻塞证据和自动修复判断；默认只显示 stage/agent/returncode/证据文件，不展开 reviewer/tester/terminal stdout/stderr；`run_external_research` / `return_to_code_execution` / `return_to_deployment` / `fix_memory_writeback` / `fix_git_publish` 会自动回流最多 2 次，高风险凭证、真实交易、资金或破坏性数据操作仍停人工确认。
