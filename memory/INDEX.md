...
- 最新 nofx 安装记录：2026-04-28 14:40 已把 runtime 代码批次 `3a44f0b0` 安装到 `/home/arbops/.hermes/ops`；后续文档/记忆记录提交只需让 nofx 仓库继续 fast-forward 到 `origin/main`，不改变本批 runtime artifact。安装态与仓库源码 hash 对齐，包含 `pipeline_runner.py`、`smart_arb_pipeline_entry.py`、`smart_arb_live_bridge.py` 的 DeliveryPlan / revise_solution 修复版本。两个 live profile `SOUL.md` 已同步仓库模板并重启 `hermes-discord-arbitrage`、`hermes-discord-spread`；远端 `compileall`、67 项定向单测、`smart-arb-pipeline --help`、gateway state 和内控 API `127.0.0.1:18080` smoke 均通过，详见 `RUNBOOK.md` / `DEPLOYMENT.md`。
