...
- 2026-04-28 本仓 nofx Discord profile 模板已升级为普通沟通/独立协作与高权限工作流维护分流：普通业务代码修改、依赖安装、业务重启、业务部署和业务提交推送仍走 coordinator pipeline；但修复 hardflow workflow/runtime/profile 本身时，profile 不再递归启动同一条 `smart-arb-pipeline`，而是进入高权限工作流维护模式，直接切到 `/home/arbops/projects/openclaw-hardflow-backup-20260302` 修改工作流宿主、测试、安装 runtime，并在无法启动独立 reviewer 时标记 `review=pending_external`。本地验证已通过；live nofx profile 仍需在 SSH 恢复且无活跃 pipeline 时同步并重启 gateway。
...
- 最新 nofx 安装记录：2026-04-28 19:40 已把最新 `origin/main` 提交 `17d9b369` 对齐并安装到 `/home/arbops/.hermes/ops`，远端 hardflow 仓库 HEAD 为 `17d9b36`，`HEAD...origin/main=0 0` 且工作树 clean。runtime installer 返回 `ok=true`、`changed=true`；3 个核心 ops 脚本仓库源码与安装态 SHA256 一致；远端 76 项定向单测、`compileall`、`/home/arbops/.local/bin/smart-arb-pipeline --help`、cron job 数 12 / `memtidy_hits=0`、gateway state、内控 API 和 echo smoke `install-smoke-arbitrageagent-20260428T114016095602Z` 均通过。本轮没有 profile SOUL 改动，因此未重启 Discord gateway；详见 `DEPLOYMENT.md`、`RUNBOOK.md`、`TASK_HISTORY.md`。
