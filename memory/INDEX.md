...
- 2026-04-28 17:01 nofx 两个 Discord profile 已上线普通沟通/独立协作三分流：用户明确说“不走工作流 / 绕过工作流 / 直接沟通 / 先讨论 / 先自己开发 / 这次不用自动流程”时不启动 `smart-arb-pipeline`，只做沟通、澄清、只读查询、状态结论和方案说明；真实代码修改、依赖安装、重启、部署、提交推送或生产配置变更仍必须由外部 Codex/SSH operator 处理，或重新授权进入 coordinator pipeline。已同步仓库模板和 live profile，并重启 `hermes-discord-arbitrage` / `hermes-discord-spread`。
...
- 最新 nofx 安装记录：2026-04-28 19:05 已把 workflow 代码批次 `353f420d` 安装到 `/home/arbops/.hermes/ops`，远端 hardflow 仓库 HEAD 为 `353f420`。安装态 `pipeline_runner.py` 与仓库源码 SHA256 一致；远端 73 项定向单测、`/home/arbops/.local/bin/smart-arb-pipeline --help`、cron job 数 12 / `memtidy_hits=0`、gateway state 和 runtime dry-run smoke `/tmp/hardflow-install-smoke-20260428T110456Z` 均通过。该批次没有 profile SOUL 改动，因此未重启 Discord gateway；详见 `DEPLOYMENT.md`、`RUNBOOK.md`、`TASK_HISTORY.md`。
