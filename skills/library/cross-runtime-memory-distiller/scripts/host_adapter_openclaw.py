#!/usr/bin/env python3
"""OpenClaw 宿主适配器：负责封装 Parser Agent 请求。"""
...
def build_parser_request(
    ...
    """把 OpenClaw 宿主探测结果和候选封包整理成统一请求。"""
    if runtime_result.host != "openclaw":
        raise ValueError("runtime_host_mismatch:expected=openclaw")
    if packet.host != "openclaw":
        raise ValueError("packet_host_mismatch:expected=openclaw")

    return {
        "host": "openclaw",
        "agent": "openclaw-parser",
