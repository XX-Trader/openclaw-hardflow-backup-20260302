...
> 2026-04-24 裁决：旧 `install_workflow_profile.py`、`workflow_setup.py` 与 `cron_setup.py / install_*_job.py` 链路已删除。它们不能再作为安装、重装或 profile 对齐入口；新入口统一为 Phase 6 的 `project-delivery-pipeline/scripts/runtime_installer.py`。
...
| 通用 runtime 安装 | `python setup.py --runtime-home <runtime_home> --runtime-name <runtime_name>` | 委托 `runtime_installer.py`，OpenClaw/Hermes 只是路径示例，不是硬编码边界 |
...
| 旧入口 | 处理方式 | 原因 |
|--------|----------|------|
| `install_workflow_profile.py` | 已删除，不保留兼容入口 | 旧实现依赖已删除的 `cron_setup.py / install_*_job.py` |
...
9. 已实现通用 runtime adapter/installer，只处理路径、runtime home、job payload、状态目录、任务中心和记忆目录。
10. 增加 dry-run 集成测试，验证状态机产物、阻断、回退和任意 runtime home 安装。

## 4. 明确不用做什么

1. 不恢复 `cron_setup.py`。
2. 不恢复 `install_*_job.py` 安装器链。
3. 不维护 OpenClaw、Hermes 或其他 runtime 的多套业务流程。
