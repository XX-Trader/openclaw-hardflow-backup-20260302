...
KNOWN_SCHEDULE_METADATA: dict[str, dict[str, Any]] = {
    ...
    "ops_maintenance_weekly_todo": {
        "owner_agent": "ops-agent",
        "executor_agent": "ops-agent",
        "capability": "self_reflection_packaging",
        "purpose": "周度复盘并生成自进化任务包",
        "flow_summary": "todo_patrol.py -> 评分 -> build TODO package",
        ...
        "maintenance_entry": "cron_setup.py / todo_patrol.py",
    },
    "ops_upgrade_feedback_daily": {
        "owner_agent": "ops-agent",
        "executor_agent": "ops-agent",
        "capability": "upgrade_feedback_analysis",
        "purpose": "汇总 executor runs，生成 workflow scorecard 与 skill review，给下一轮升级提供 baseline/candidate 对比。",
        "flow_summary": "upgrade_feedback_runner.py -> 切 baseline/candidate -> workflow_upgrade_scoring.py + skill_evolution_review.py -> 写入报告目录",
        "required_skills_or_runtime": ["task_center executor reports", "workflow_upgrade_scoring.py", "skill_evolution_review.py"],
        "outputs": ["workflow scorecard", "skill evolution review", "upgrade feedback summary"],
        "human_visibility": "默认静默，供升级技能与运维巡检读取",
        "delivery_mode": "none",
        "health_signals": ["candidate 窗口切分成功", "scorecard 生成成功", "state 去重正常"],
        "failure_signals": ["executor reports insufficient", "scorecard render failed", "state write failed"],
        "maintenance_entry": "cron_setup.py / upgrade_feedback_runner.py",
    },
