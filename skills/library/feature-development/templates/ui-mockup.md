...
**页面路径**: `/pm-robot/member-group`

**功能概述**: 管理成员分组，支持创建、编辑、删除分组，以及添加成员到分组
...
前端验证（名称不为空、长度不超过100）
  ↓
调用API: POST /api/pm-robot/member-group/create
  ↓
成功：关闭对话框，刷新列表
失败：显示错误提示
```
...
前端验证（至少选择1个，检查是否已在其他分组）
  ↓
调用API: POST /api/pm-robot/member-group/add-members
  ↓
成功：关闭对话框，刷新列表，显示"成功添加X个成员"
失败：显示失败原因（如"成员XX已在YY分组"）
```
...
用户点击"确定"
  ↓
调用API: POST /api/pm-robot/member-group/batch-start
  ↓
显示加载状态："正在启动..."
  ↓
成功：显示"成功启动3个成员"，刷新列表
...
| 字段 | 类型 | 必填 | 验证规则 | 说明 |
|------|------|------|----------|------|
| 成员列表 | 穿梭框(Transfer) | ✅ | 至少选择1个 | 左侧显示所有未分组成员 |
...
| AddMemberDialog.vue | 添加成员对话框 | src/components/pm-robot/ |
| MemberTable.vue | 成员表格组件 | src/components/pm-robot/ |
