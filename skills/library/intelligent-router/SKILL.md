...
在调用方已经掌握 Skill 目录、Agent 注册表和任务上下文时，用本 Skill 生成一个结构化路由决策。目标 Runtime 的安装状态、Agent 启动、并行编排、结果合并和业务验收由调用方负责。
...
1. 先检查三种显式格式：
   ...
   - `[调用组合: <combo>] <task>`
2. 校验显式目标：Skill 必须有 `SKILL.md`，Agent 必须处于注册表可用集合，组合必须在允许列表中。
...
- 注册表只是一层静态声明；真正分发前还要核对 Runtime 的实际 Agent 清单。
