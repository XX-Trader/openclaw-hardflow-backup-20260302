...
- [x] [2026-04-29] **specified_agent 指定 agent 路线与 session/run id 状态卡上线**
  ...
  - 代码批次 `22cecab` 已安装到 nofx，最终仓库提交 `939288b` 已拉到 nofx；远端 `smart-arb-pipeline --route-choice specified_agent --assignee tester` smoke 通过，状态卡展示被调用 agent、Task Center id、executor run id、agent session id、agent run id、session key、当前阶段、完成状态和失败原因。
