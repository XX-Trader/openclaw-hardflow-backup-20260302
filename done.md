...
- [x] [2026-04-28] **DeliveryPlan 结构化方案契约与 revise_solution 自动回流**
  ...
  - 验证：本地 `python -B -m unittest tests.scripts_openclaw_ops.test_project_delivery_pipeline_runner tests.scripts_openclaw_ops.test_smart_arb_pipeline_entry tests.scripts_openclaw_ops.test_smart_arb_live_bridge` 96 项 OK；nofx 已安装 `89bba4af`，远端 `compileall` 与 62 项定向单测 OK。
...
- [x] [2026-04-28] **nofx Discord 证据短标签与 cron 群投递**
  ...
  - 后续部署已把包含该变更的 `89bba4af` 安装到 nofx live runtime；两个 live profile `SOUL.md` 已同步仓库模板并重启，gateway 均为 `running/connected`。
...
- [x] [2026-04-28] **nofx 拉取并安装 `89bba4af` hardflow runtime**
  - 本机提交 `89bba4af` 已推送到 `origin/main`；nofx 仓库已对齐该提交，工作树 clean，`HEAD...origin/main=0 0`。
  ...
  - 两个 live profile `SOUL.md` 已同步仓库模板，备份为 `SOUL.md.bak-20260428T142632`，并重启 `hermes-discord-arbitrage`、`hermes-discord-spread`。
  - 验证：远端 `compileall` 通过；定向单测 62 项 OK；`smart-arb-pipeline --help` 正常；两个 gateway `running/connected`；内控 API `/health` 与 `/api/strategy/status` 通过。
