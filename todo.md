...
- [x] [🟡 P1] 将 specified_agent/session-run-id 批次推送并安装到 nofx live runtime：提交 `22cecab` 已推送并在 nofx `git pull --ff-only`，runtime installer `ok=true/changed=true`，远端指定 agent smoke 返回 Task Center、executor run id、agent session id、agent run id、session key 和 `回答状态: 已回答完毕`
