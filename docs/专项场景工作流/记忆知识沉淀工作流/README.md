...
本工作流是**蒸馏结果的消费方**，不是“多源数据接入、热记忆设计、检索层设计、LLM 解析门禁”的主文档。

边界必须明确：

- **本工作流负责**：消费蒸馏结果，做治理、技能候选提炼、行为优化、TODO 生成。
- **本工作流不负责**：定义 transcript 从哪里抓、如何切片、哪些片段才允许进 LLM、热记忆写到哪里。

上述基础设施裁决统一以 `docs/基础设施/记忆蒸馏/Hermes-风格记忆蒸馏升级/` 为准。
