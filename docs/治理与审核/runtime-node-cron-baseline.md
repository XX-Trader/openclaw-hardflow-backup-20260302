# 2026-03-17 HOST_A Cron 基线

## 目标

给 `HOST_A` 这类“线上运行节点 + Telegram 交互入口 + 核心运维执行机”定义一套更稳的 cron 基线，避免把它同时当成重度 reviewer 机和全自动进化机。
...
`HOST_A` 的任务执行器是高频任务。如果每 10 分钟都把同一批未闭环问题完整重发，会直接淹没人能消费的信息。因此通知策略调整为：
