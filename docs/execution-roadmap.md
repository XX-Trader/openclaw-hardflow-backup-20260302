...
| # | 任务 | 预计工时 | 依赖 |
|---|------|---------|------|
| 6.1 | nofx 全功能验收测试 | 2h | 阶段五完成 |
| 6.2 | 推广到其余 4 台服务器 | 4h | 6.1 |
| 6.3 | Lobster 仓库配置为 `external_readonly` | 0.5h | 无 |
| 6.4 | 拆分 `policy_enforcer.py`（270KB）为独立模块 | 8h | 6.1 |
| 6.5 | Agent 模型配置同步（coordinator→gpt-5.4 等 4 项） | 1h | 无 |
