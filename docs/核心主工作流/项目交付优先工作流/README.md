...
> 2026-04-27 nofx 运行态口径：服务器 live 入口是 `arbitrageagent` 与 `spreadagent` 两个 Hermes Discord profile；2026-05-08 已补充模型路由：主模型均为 `openai-codex/gpt-5.5`，主回退链为 `kimi-coding/kimi-k2.6 -> zai/glm-5.1`，辅助任务默认 `zai/glm-4.7`，`compression/curator` 使用 `zai/glm-5.1`，运行态不再配置 OpenRouter。执行链路是 `/home/arbops/.local/bin/smart-arb-pipeline -> /home/arbops/.hermes/ops/pipeline_runner.py`；`coordinator`、`project-agent`、`web-agent`、`reviewer`、`backend-dev`、`frontend-dev`、`tester`、`deployer`、`doc-writer` 是 workflow 阶段 owner / workspace 标签，不是 nofx 上 14 个常驻 agent。
...
> 2026-05-06 高风险确认贯通（历史口径，已被 2026-05-10 门禁收口覆盖）：真实交易、下单、划转、提现和资金类策略需求曾需要 `--human-risk-confirmed` 通过 `risk_gate`；当前只把该参数作为历史兼容/审计字段，不决定是否继续。
...
> 2026-05-07 方案门禁收口：`delivery_plan.json` 会过滤 workflow 宿主 basename、低信任 `scripts/openclaw-ops/*`、组合文件路径和否定上下文路径；每个保留目标生成文件级实施步骤。方案 reviewer 未通过时必须把失败原因合并回修订计划并重新审核。
> 2026-05-08 执行保护契约为历史口径：当时用 `execution_guard.json` 替代高权限关键词硬停。2026-05-10 起该契约已移除。
> 2026-05-10 门禁收口：真实交易、下单、划转、提现等业务动作和普通执行动作不再作为 workflow 风险门禁，也不再生成 `execution_guard.json`；`pre_execution_risk.json` 只保留执行前记录。需求审核、方案审核、验证、代码审核、deployment 和 git_publish 失败必须回流给开发/发布修复并重新审核/上传，直到通过。唯一保留的密码/密钥硬检查在 Git 发布 staged diff 扫描。
...
13. **受控 Git 发布**：只有验证、代码审查、deployment（如有）、验收和记忆回写通过后才允许 `git_publish`；发布输入优先采用 `memory_writeback` 隔离工作区 patch，缺失时只回退到已验收的 `code_execution` patch，确保代码与文档/记忆回写作为同一个已验收变更集发布且不夹带未验收脏改动；提交说明、备注和变更描述必须使用中文并脱敏。发布前只扫描 staged diff 的新增行：真实 token、真实 cookie、OAuth secret、交易所 key、`.env` 实值和高熵随机串 hard block；环境变量名、空值、`os.getenv(...)`、README 占位说明和测试假密码不应误报。发布失败必须进入 `fix_git_publish` 回流修复，不再 warning 继续。
...
16. **业务动作不设 workflow 门禁**：交易、下单、提现、划转等业务动作不再进入 `risk_gate` 或 `execution_guard.json`。流水线只负责把需求、方案、开发、验证、审核、部署、发布串成失败回流闭环；业务动作是否正确由实现、测试和 reviewer 判断。密码/密钥类上传保护只保留在 Git 发布 staged diff 扫描。
...
| 执行可控 | 每个阶段都有 pass signal 和失败回退动作；审核、验证、部署和发布失败必须带失败原因回流修复，不允许跳过审查、测试或 Git 发布密码扫描 |
