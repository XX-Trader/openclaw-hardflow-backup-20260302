...
> 2026-05-06 高风险确认贯通（历史口径，已被 2026-05-08 执行保护契约覆盖）：真实交易、下单、划转、提现和资金类策略需求曾需要 `--human-risk-confirmed` 通过 `risk_gate`；当前只把该参数作为确认来源留痕。
...
> 2026-05-07 方案门禁收口：`delivery_plan.json` 会过滤 workflow 宿主 basename、低信任 `scripts/openclaw-ops/*`、组合文件路径和否定上下文路径；每个保留目标生成文件级实施步骤。Graphify / pre-execution 风险扫描只看可执行意图，不把安全边界、状态摘要或验证命令中的关键词当成真实高权限动作；正向凭证 hard block，真实交易、下单、资金动作进入 `execution_guard.json`。
> 2026-05-08 执行保护契约：真实交易、下单、撤单、划转、提现、force push、删除、覆盖和数据库破坏性操作不再因关键词停在 `risk_gate`；命中后生成 `execution_guard.json`，按小额/白名单/幂等/审计/回读和备份/恢复约束继续。硬停只保留凭证泄露、目标不明确、备份或审计准备失败。
...
13. **受控 Git 发布**：只有验证、代码审查、deployment（如有）、验收和记忆回写通过后才允许 `git_publish`；发布输入优先采用 `memory_writeback` 隔离工作区 patch，缺失时只回退到已验收的 `code_execution` patch，确保代码与文档/记忆写回作为同一个已验收变更集发布且不夹带未验收脏改动；提交说明、备注和变更描述必须使用中文并脱敏。默认不做 force push；如用户明确要求，必须先通过 `execution_guard.json` 记录目标分支、备份/回滚路径和审计证据。发布前只扫描 staged diff 的新增行：真实 token、真实 cookie、OAuth secret、交易所 key、`.env` 实值和高熵随机串 hard block；环境变量名、空值、`os.getenv(...)`、README 占位说明和测试假密码不应误报。
...
16. **执行保护契约**：交易、下单、提现、划转、force push、删除、覆盖、数据库 drop/truncate/delete 等高权限动作进入 `execution_guard.json`，按最小金额/小额、白名单、幂等、审计、回读、备份、恢复和保留 TTL 约束继续；硬停只保留凭证泄露、目标不明确、备份或审计准备失败。`--human-risk-confirmed` 只作为确认来源留痕，不再作为这些关键词的前置放行条件。
...
| 执行可控 | 每个阶段都有 pass signal 和失败回退动作；高权限动作用 `execution_guard.json` 约束，不允许跳过审查、测试、备份、审计或回读 |
