...
该 runner 不替代人工确认队列，也不直接修改 TODO 文件。它只把已经在 Task Center 内可执行的 backlog 转交给统一项目交付流水线：低/中风险可直接推进；高风险必须已在 Task Center 记录 `human_confirmed=true` 且 runner 显式启用 `--allow-confirmed-high-risk`，随后传递 `--human-risk-confirmed` 给入口和 pipeline runner，避免人工确认后再次卡在同一个 `risk_gate`。
