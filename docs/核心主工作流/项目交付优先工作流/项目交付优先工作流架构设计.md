...
graph TD
    ...
    E --> F{双 AI 需求审查}
    F -->|需要补充| C
    F -->|通过| G[Solution Package：DeliveryPlan 契约 / Markdown 展示]
    G --> H{双 AI 方案审查}
    H -->|需要修订| G
...
|----------|----------|------------------|----------|
| 到期 TODO | `skills/library/todo-patrol/scripts/deadline_to_task_bridge.py` | 创建 `todo_deadline_candidate`；默认写入 `route_selection.mode=manual_selection` 并进入 `human-inbox` | 系统推荐链路，用户选择后才执行；只有 pipeline 类选择由 backlog runner 推进 |
...
2. `todo_deadline_to_task_bridge_daily`：每日 00:05 把到期 TODO 转为人工路线选择候选。
3. `system_exception_patrol`：继续负责异常巡检报告。
4. `system_exception_to_task_bridge`：每 6 小时把异常巡检结果转为运维任务。

### 2.0.2 Task Center 待办持续推进

2026-04-27 服务器聊天记录复核显示，runtime-host 上真实链路虽然能通过 `projectagent` 人工触发 `project-delivery-pipeline`，但没有从 Task Center 自动取待办继续推进的执行器。新增 `scripts/openclaw-ops/backlog_runner.py` 作为受控续跑器：
...
| 安全门禁 | `need_human_confirm`、`needs_clarification`、未确认高风险、`escalate_human` 一律跳过，继续停在 `human_inbox.py`；已确认高风险必须同时传入 `--allow-confirmed-high-risk` 和 `--human-risk-confirmed` 才能继续 |
| 防循环 | 默认每个任务只记录 1 次 `backlog_runner_attempt`，达到 `--max-attempts-per-task` 后不再重复续跑 |
| 执行方式 | 调用 runtime 内安装的 `python3 <runtime_home>/ops/project_pipeline_entry.py --live --profile projectagent --source backlog-runner --emit-json --requirement <安全包>`，后续仍走需求、编码、验证、代码审查、deployment、记忆回写和 Git 发布门禁 |

该 runner 不替代人工确认队列，也不直接修改 TODO 文件。它只把已经在 Task Center 内可执行的 backlog 转交给统一项目交付流水线。2026-05-10 起业务动作关键词不再触发 `risk_gate` 或 `execution_guard.json`；`--human-risk-confirmed` 只作为历史兼容/审计字段，实际阻塞应来自需求不清、审核未通过、验证失败、deployment 失败或 Git 发布密码/密钥扫描失败。
...
| `requirements_review` | 双 AI 对抗式审查需求合理性 | `requirements_review.md` + `command_requirements_review_1/2.json` | 优先两路不同模型 reviewer；provider/model 失败时走 fallback，至少一个有效 reviewer 裁决 `ready_for_solution` 且无明确 blocker |
| `solution_package` | 编译结构化交付计划，Markdown 仅作为人工展示 | `delivery_plan.json` + `solution.md` | 契约包含切片、目标文件/定位策略、实施步骤、验证命令、发布/回滚门禁和人工阻塞条件 |
| `solution_review` | 双 AI 对抗式审查交付计划 | `solution_review.md` + `command_solution_review_1/2.json` | 优先两路不同模型 reviewer 先审 `delivery_plan.json`；provider/model 失败时走 fallback，至少一个有效 reviewer 裁决 `ready_for_implement` 且无明确 blocker |
...
| `code_review` | 双 AI 审核代码是否按方案执行 | `code_review.md` + `command_code_review_1/2.json` | 优先两路不同模型 reviewer；provider/model 失败时走 fallback，至少一个有效 reviewer 裁决 `pass` 且无明确 blocker |
...
2. 不能在未获确认时执行破坏性命令、生产写操作、密钥变更或外部发布。
...
| 待办续跑 | `backlog_runner.py --pipeline-command <project-delivery-pipeline>` | `task_outputs.output_type=backlog_runner_attempt` + pipeline run id |
...
2026-04-24 实测：WSL `/home/runtime-user/.hermes` 上 `hybrid-single-chat` smoke 通过，run_id=`hermes-profile-smoke-20260424T135014Z`，Task Center task=`project-delivery:hermes-profile-smoke-20260424T135014Z`。旧逐段调用路径中 research/code/review 分别耗时数分钟，verification 曾超过 300 秒；因此默认 smoke 已改为单次 bundle。
...
1. 避免把阶段 owner 继续注册成常驻 Agent；runtime-host 当前 live 入口实测只有 `deliveryagent` / `projectagent` 两个 Hermes profile，阶段 owner 是逻辑标签与隔离 workspace，不等于 14 个常驻 agent
...
| Solution Package | `project-agent` | `delivery_plan.json` 结构化交付契约；`solution.md` 只是人工可读渲染 |
...
| 方案 Markdown 泛化 | reviewer 反复要求修订，无法进入编码 | 以 `delivery_plan.json` 作为事实源，reviewer/code agent 审查和执行结构化契约 |
...
| 自动续跑失控 | 低风险待办重复失败、绕过人工确认或无限重试 | backlog runner 只取安全来源、跳过人工/澄清项、默认单任务单次尝试，所有执行仍走 `project-delivery-pipeline` 的审核、验证、deployment、Git 发布回流门禁 |
