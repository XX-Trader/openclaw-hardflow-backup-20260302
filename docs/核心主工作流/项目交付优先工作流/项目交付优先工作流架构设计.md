...
> 版本：v0.5 | 2026-04-24（补齐运营事件入任务中心与人工队列闭环）
...
|----------|----------|------------------|----------|
| 到期 TODO | `skills/library/todo-patrol/scripts/deadline_to_task_bridge.py` | 创建 `todo_deadline_candidate`，`need_human_confirm=true`，默认进入 `human-inbox` | 用户确认后才进入可执行队列，拒绝则取消 |
| 增量异常日志 | `skills/library/log-monitor/scripts/exception_to_task_bridge.py` | 按异常 fingerprint 去重创建 `ops_exception` 任务，并写 `task_incidents` | `critical` 级别默认等待人工确认，普通异常交给 `ops-agent` |
...
2. `todo_deadline_to_task_bridge_daily`：每日 00:05 把到期 TODO 转为待人工确认候选任务。
...
| `code_review` | 双 AI 审核代码是否按方案执行 | `code_review.md` | 裁决为 `pass` |
...
`repo_hygiene_reviewer.py` 是只读巡检器，由 `optimization-agent` 每 2 天触发一次。它只负责发现候选清理项，不直接删除文件、不直接提交 Git、不自动部署。
