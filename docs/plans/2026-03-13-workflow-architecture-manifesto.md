...
| `project_index_maintainer_30m`<br>`5797cd5b...` | `openclaw_cron` | `project-agent` | `project-agent` | 维护项目索引与文档知识 | `every 30m` | `project_index_maintainer.py -> registry -> project-index-local/doc-knowledge` | `project-registry.json`、`task_center.db`、文档抓取、索引写入 | `project-index-local/*`、知识索引、运行绑定 | 一般静默；异常由巡检汇总 | `none` | 索引文件刷新、知识条目增长、空闲 `NO_REPLY` | `network_error`、git/index drift、文档抓取失败 | `install_project_index_job.py`、`policy/project_index_maintainer.py` |
...
| `ops_daily_summary`<br>`621ee42b...` | `openclaw_cron` | `ops-agent` | `ops-agent` | 每日巡检总结 | `cron: 5 0 * * *` | `ops_cron_runner.py --mode daily` | `ops_cron_runner.py`、巡检历史 | 每日汇总、风险概览 | 仅有有效内容时可见 | `announce` | 日报可生成、留痕完整 | 生成超时、摘要为空但未静默 | `cron_setup.py`、`ops_cron_runner.py` |
...
| `ops_daily_work_report_dingtalk`<br>`9873ab34...` | `openclaw_cron` | `ops-agent` | `ops-agent` | 每日 todo/done 增量去重并发送钉钉 | `cron: 15 0 * * *` | `daily_work_report.py -> load tasks -> build digest -> post_dingtalk` | `task_center.db`、`runtime.env`、`DINGTALK_WEBHOOK_URL`、`DINGTALK_SECRET`、`lightContext` | 日报 JSON、钉钉投递、聊天摘要 | 每日可见；无新增且无异常时应静默 | `announce` | `dingtalk_ok`、增量去重成功、报告生成成功 | `webhook_missing`、`dingtalk_post_failed`、运行超时 | `cron_setup.py`、`daily_work_report.py` |
...
| `reviewer_git_update_hourly`<br>`d3859fd5...` | `openclaw_cron` | `reviewer` | `reviewer` | 小时级 git 增量扫描、PR 检查、可选合并 | `every 1h` | `reviewer_cron_runner.py --mode hourly_git` | reviewer runner、workspace、可选 merge approval、`lightContext` | Git 审查结果、PR 检查记录 | 有发现或失败时可见 | `announce` | 扫描完成、PR 检查成功、合并规则生效 | Git 读取失败、审批缺失、超时 | `install_reviewer_scan_jobs.py`、`reviewer_cron_runner.py` |
...
- 负责任务：`project_index_maintainer_30m`、`web_intel_review_project_docs_6h`。
